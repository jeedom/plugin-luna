	...
	<fieldset>
		...
		<div class="row">
			<div class="col-sm-6">
				...
				<span>{{La mise à jour du Recovery, permet de telecharger la}} <b>{{derniere}}</b> {{image de restauration de votre Luna.}} <br /> {{Mais aussi de mettre a jour (via cette image) les composants Linux de celle-ci.}} <br /> {{Votre Luna doit etre}} <b>{{connectée}}</b> {{à internet pour effectuer cette operation.}}</span>
				<br /><br />
				<center>
			...
			<div class="col-sm-6">
				...
				<span>{{Le recovery, permet une}} <b>{{remise a zero d'usine}}</b> {{de la Luna avec l'image inclus dans celle-ci (vous pouvez la mettre à jour via Mise à jour du recovery).}} <br/> {{Attention a bien}} <b>{{sauvegarder}}</b> {{votre box et recuperer celle-ci sur votre ordinateur avant de lancer le recovery.}}</span>
				<br /><br />
				<center>
